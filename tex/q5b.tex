...
of the framework.
  ...
  \section{Status Table and Problem Resolution}
  \label{sec:status}
  ...
  \subsection{Resolved problems}
    ...
    Combined with $A_H = 2$ (Q5b-O3, resolved in Q11~\cite{Beau2026q11}), the effective
    co-metric is $g^{\mu\nu} = \mathrm{diag}(-A_\tau,\, 2,\, 2,\, 2)$;
    $A_\tau = 2$ is subsequently established in Q11~\cite{Beau2026q11},
    closing the metric to $g^{\mu\nu} = 2\,\eta^{\mu\nu}$.
    ...
    The identification $A_H = 2$ was established by Q10~\cite{Beau2026q10} via
    ...
    The identification of $A_\tau = 2$ is established in Q11~\cite{Beau2026q11}
    via a cascade internalisation lemma: the derivative $\partial_\tau$ is the continuum
    limit of the cascade increment operator $T : \sigma_c(n) \mapsto \sigma_c(n+1)$
    by the Carnot--Carath\'eodory convergence of Theorem~\ref{thm:carnot}.
    The SU(2)-equivariance of $T$ under~[U] then forces, by Schur's lemma and the
    Casimir normalisation of Q8~\cite{BeauQ8},
    \[
      A_\tau = C_{\mathfrak{su}(2)}\big|_{\mathrm{Sym}^2(V_\rho)} = 2.
    \]
    Open problem Q5b-O3 is therefore closed.
    The effective co-metric is uniquely determined with no free parameter:
    \[
