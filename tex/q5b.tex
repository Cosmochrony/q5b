...
\begin{document}
  ...
  \begin{abstract}
    ...
    conditional, imported from~\cite{Beau2026c}.
  \end{abstract}
  ...
  on the hyperbolicity hypothesis of~\cite{Beau2026c}).}
...
of~\cite{Beau2026c}, not derived from the capacity bound.
  ...
  \subsection{Ancillary questions and their resolution}
    ...
    no free parameter, and, conditionally on the hyperbolicity hypothesis [H-hyp]
    of~\cite{Beau2026c} (Theorem~\ref{thm:lorentz}), the signature is Lorentzian:
    \[
      g^{\mu\nu} = 2\,\eta^{\mu\nu}, \qquad \eta = \mathrm{diag}(-1,+1,+1,+1).
    \]
    Spacetime structure is, under this hypothesis, a derived consequence of the
    non-injective projection, not an input.
