...
\begin{document}
  ...
  \begin{abstract}
    Earlier versions of the companion paper Q5a claimed, conditionally, that the admissibility
    forms $\Eq$ on the Weil representation of $\HeisZ$ converge in the Mosco sense to a
    second-order operator $\LPi = -A\partial_x^2$ on $L^2(\mathbb{R})$.
    Version 3.0 of Q5a withdraws that derivation: the canonical filtration of the admissible
    fibre is exactly a growing toric Fourier window, the published admissibility form converges
    to the zero form on it, and no common scalar normalisation of the form produces a
    non-trivial toric differential operator.
    The spatial input of the present paper is therefore not established; we formalise it as an
    explicit hypothesis [H-L] (existence of a spatial second-order limit operator
    $\LPi = -A\partial_x^2$ on $L^2(\mathbb{R})$) and state every result that consumes it
    conditionally on [H-L].
    ...
    Second, under [H-L] we identify $\LPi$ as the image, under the Schr\"{o}dinger
    representation, of the kinetic sector of the sub-Riemannian Laplacian $\DeltaH$ on
    $\HeisR$, and extract an effective co-metric tensor from the principal symbol of the full
    effective operator (Theorem~\ref{thm:metric}).
    The lifting hypothesis~[H-lift] has been proved in Q9~\cite{Beau2026q9}, so the
    conditionality of Theorem~\ref{thm:metric} reduces to [H-L] itself.
    The metric coefficients are determined by companion papers: $A_H = 2$ by
    Q10~\cite{Beau2026q10}, $A_z = 2$ by Q8~\cite{BeauQ8}, and $A_\tau = 2$ by
    Q11~\cite{Beau2026q11}, giving, still under [H-L], the isotropic Lorentzian co-metric
    $g^{\mu\nu} = 2\,\eta^{\mu\nu}$ with no remaining free parameter.
    Third, the Born--Infeld admissibility constraint conditionally selects the Lorentzian
    signature $(-,+,+,+)$ for this metric (Theorem~\ref{thm:lorentz}), the import being
    conditional on the hyperbolicity hypothesis of~\cite{Beau2026c} ([H-hyp]).
    Each result is given with an explicit status: structural, or conditional on [H-L] (and,
    where indicated, on [H-hyp]).
    The only potential source of [H-L] currently identified in the corpus is the conditional
    dual-window regime of Q5a version 3.0, which requires an asymptotically non-vanishing
    critical coverage --- a scenario the published data contradict; Q5 remains open.
  \end{abstract}
  ...
  \subsection{Context and the status of the Q5a input}
    ...
    Its earlier versions claimed, conditionally, a Hilbert inductive limit, convergence of
    rescaled Weil generators to metaplectic generators, and Mosco convergence of the
    admissibility forms $\Eq$ to a second-order operator $\LPi = -A\partial_x^2$ on
    $L^2(\mathbb{R})$, and deferred the geometric reading of $\LPi$ to the present paper.
    Version 3.0 of Q5a withdraws that derivation.
    It proves that, with the pipeline's initial vector, the canonical filtration of the
    admissible fibre is exactly a growing toric Fourier window; that the published
    admissibility form converges to the zero form on this filtration; and that, if the
    saturation depth diverges and the measured weights have positive finite limits, no common
    scalar normalisation of the form produces a non-trivial toric differential operator.
    In particular, no operator $-A\partial_x^2$ on a spatial $L^2$ space is currently derived
    anywhere in the corpus, and question Q5 is open.

    The spatial input of this paper is therefore an explicit hypothesis:
    \begin{hypothesis}[{[H-L]}: spatial limit input; not established]
      \label{hyp:HL}
      There exists a normalisation of the admissibility structure under which the
      admissibility forms converge to a second-order operator $\LPi = -A\partial_x^2$ on
      $L^2(\mathbb{R})$ with $A > 0$.
    \end{hypothesis}
    Hypothesis~[H-L] is not established; Q5a version 3.0 shows that it fails for the published
    normalisation and for every common scalar rescaling of it on toric targets.
    The only potential source of [H-L] currently identified in the corpus is the conditional
    dual-window regime of Q5a 3.0, which would require the critical coverage $x_1(q)$ to remain
    bounded away from zero --- a scenario the published data
    contradict~\cite{Beau2026ccnote}.
    All results below that consume $\LPi$ are stated conditionally on [H-L]; the geometric
    convergence results (Theorems~\ref{thm:bass}--\ref{thm:carnot}) are independent of it.
    The present paper delivers the geometric reading in this conditional form.
  ...
  \subsection{Scope and main results}
    ...
    \medskip
    \noindent\textbf{Metric extraction (Theorem~\ref{thm:metric}; conditional on [H-L],
  with the lifting step proved in Q9~\cite{Beau2026q9}).}
    Under [H-L] and Hypothesis~[H-lift] (Definition~\ref{hyp:lift}, proved in Q9),
    $\LPi = -A\partial_x^2$ is
    ...
    \medskip
    \noindent\textbf{Lorentzian signature (Theorem~\ref{thm:lorentz}; conditional on [H-L]
  and on the hyperbolicity hypothesis of~\cite{Beau2026c}).}
    The Born--Infeld admissibility constraint, together with the hyperbolicity
    hypothesis [H-hyp] of~\cite{Beau2026c}, selects
    $g^{\mu\nu} = \mathrm{diag}(-1,+1,+1,+1)$ in homogeneous isotropic admissible regimes.
  ...
  \subsection{Notation and conventions}
    ...
    The hypothetical spatial operator of [H-L] is $\LPi = -A\partial_x^2$ acting on
    $L^2(\mathbb{R})$, where $A > 0$ is an unspecified positive constant (in the withdrawn
    Q5a derivation, the admissibility weight).
...
The O-series numerical campaign (O9--O25; synthesised in~\cite{Beau2026ccnote}) confirms
$\Dhom = 4$ empirically: the fitted ball-growth exponent $\hat D$ converges to $4$
monotonically as $q$ increases across primes $q \in \{11, \ldots, 401\}$.
A pre-saturation window of polynomial ball growth is observed at every tested prime, so the
Pansu convergence of Theorem~\ref{thm:carnot} is visible in the data.
    \end{remark}
  ...
  \subsection{The Schr\"{o}dinger representation and the 1D shadow}
    ...
    Hypothesis~[H-L], however, posits a \emph{one-dimensional} operator
    $\LPi = -A\partial_x^2$ acting on $L^2(\mathbb{R})$ (no longer derived by Q5a; see
  ...
  \subsection{Admissibility filter and sector dominance}
    ...
    \end{itemize}
    Under [H-L], the admissibility forms would converge to
    $A\int |\partial_x f|^2\,dx$, capturing only the kinetic sector.
    ...
    This behaviour is consistent with the convergence posited by [H-L]:
    Mosco convergence suppresses oscillatory contributions and selects
  ...
  \subsection{The lifting hypothesis}
    ...
    \begin{hypothesis}
      [[H-lift]]
      \label{hyp:lift}
      The operator $\LPi = -A\partial_x^2$ posited by Hypothesis~[H-L]
      (Hypothesis~\ref{hyp:HL}) is the image, under the Schr\"{o}dinger representation
    ...
    \begin{remark}
      ...
      \begin{enumerate}
        \item Earlier versions of Q5a claimed (T2; withdrawn in version 3.0) that the
        rescaled Weil generators $\hat X_q / \sqrt{q}$, $\hat Y_q / \sqrt{q}$ converge
        strongly to the metaplectic generators $d\pi_1(\tilde X) = \partial_k$ and
        $d\pi_1(\tilde Y) = ik$, which are precisely the generators entering $\DeltaH$
        via~\eqref{eq:generators-rep}; this is retained here as structural motivation only.
        \item The limit posited by [H-L] captures only the kinetic sector, consistently with
        ...
        $\xi$ and, in the withdrawn Q5a derivation, the admissibility weight).
      \end{enumerate}
  ...
  \subsection{The full effective operator on $\mathbb{R}_\tau \times \HeisR$}
    ...
    The sign structure in~\eqref{eq:leff} reflects the hyperbolic character selected,
    conditionally on the hyperbolicity hypothesis of~\cite{Beau2026c}, in
...
of Q5a, consistently with~\cite{Beau2026q5a}.
  ...
  also~\cite{Beau2026a}), the effective metric tensor $g^{\mu\nu}(p)$ is read off
    ...
    coefficient $-A_\tau$, consistently with the hyperbolic structure hypothesised
    ...
    \begin{theorem}[Metric extraction; conditional on {[H-L]} and {[H-lift]}]
      \label{thm:metric}
      Under Hypotheses~[H-L] and [H-lift], the operator $\LPi = -A\partial_x^2$ lifts
    ...
    \begin{remark}[Unconditional status via Q9]
      \label{rem:q9lift}
      Hypothesis~[H-lift] has been proved in Q9~\cite{Beau2026q9} via generator suppression.
      The conditionality of Theorem~\ref{thm:metric} therefore reduces to Hypothesis~[H-L]
      alone: what is missing is not the lift but the existence of the spatial limit operator
      itself (Q5a version 3.0).
    \end{remark}
  ...
  \subsection{The signature selection argument}
    ...
    The following theorem imports the conditional signature-selection analysis
    of~\cite{Beau2026c}, adapted to the present geometric setting;
    no independent proof is given here.
    In~\cite{Beau2026c}, the capacity bound is an explicit axiom [A-cap] and the signature
    selection is conditional on a second-order hyperbolicity hypothesis [H-hyp]:
    the capacity bound alone does not force hyperbolicity (flux-limited diffusion is a
    counterexample).
    We state the result for completeness and give the main lines of argument to make the
    paper self-contained.

    \begin{theorem}[Lorentzian signature; conditional on Theorem~\ref{thm:metric} and on
      the hyperbolicity hypothesis {[H-hyp]} of~{\cite{Beau2026c}}]
      \label{thm:lorentz}
      In homogeneous and quasi-isotropic admissible regimes, and under the hyperbolicity
      hypothesis [H-hyp] of~\cite{Beau2026c}, the Born--Infeld admissibility
      constraint selects the Lorentzian signature $(-,+,+,+)$ for the effective metric
      defined by~\eqref{eq:op-metric}.
    \end{theorem}

    \begin{proof}[Proof sketch, following the conditional analysis of~{\cite{Beau2026c}}]
The hyperbolicity hypothesis [H-hyp] of~\cite{Beau2026c} posits that the operator
$L_{\mathrm{eff}}$ defines a second-order, well-posed initial value problem in the
projective ordering variable $\tau$; this is an explicit hypothesis, motivated by the
structural stability of the admissibility cascade~\cite{Beau2026pto,Cosmochrony}, and
is not itself a consequence of the capacity bound.
Under [H-hyp], the principal symbol $g^{\mu\nu} k_\mu k_\nu$ determines the propagation
properties of effective modes in the continuum limit, so $g^{\mu\nu}$ must be compatible
with hyperbolicity in the sense of H\"{o}rmander.

Four signature classes must be excluded:
\begin{enumerate}
  \item \textbf{Riemannian $(+,+,+,+)$}: produces a positive-definite symbol, hence an
  elliptic operator.
  An elliptic operator in $(\tau, x, y, z)$ admits no finite characteristic cone, hence
  implies instantaneous propagation of perturbations across the relational network,
  contradicting the finite capacity bound [A-cap]
  $A_n^{\max} = c_{\mathrm{BI}} / \sqrt{\lambda_n}$~\cite{Beau2026m,Beau2026c}.
  \item \textbf{Ultra-hyperbolic (two or more time-like directions)}: fails to admit a
  globally well-posed initial value problem (Hadamard ill-posedness) and is excluded under
  the hyperbolicity hypothesis of~\cite{Beau2026c}.
...
$\sigma_2(L_{\mathrm{eff}})$ (hypothesis [H-hyp]), (ii)~a finite characteristic cone,
(iii)~monotonic projective ordering along $\tau$, and (iv)~the bounded Born--Infeld
admissibility constraint, is the Lorentzian signature $(-,+,+,+)$.
The selection argument is given in~\cite{Beau2026c}, conditionally on its hyperbolicity
hypothesis, and depends, beyond that hypothesis, only on the capacity bound [A-cap]
of~\cite{Beau2026c}, whose bounded-flux content is present in the Foundation
axioms~\cite{Beau2026m}.
After normalising the maximal admissible propagation speed to $c = 1$, the effective
metric is fixed to $g^{\mu\nu} = \mathrm{diag}(-1,+1,+1,+1)$ in the
homogeneous isotropic regime.
Conditionally on [H-hyp], this selection mechanism uses only the admissibility
constraints; the hyperbolicity hypothesis itself is an explicit assumption
of~\cite{Beau2026c}, not derived from the capacity bound.
    \end{proof}

    \begin{remark}[Emergence, not postulation]
      \label{rem:emergence}
      The Lorentzian metric is not introduced as a background structure.
      Conditionally on the hyperbolicity hypothesis [H-hyp] of~\cite{Beau2026c},
      ...
      and the Born--Infeld admissibility bound (excluding the remaining signatures).
      Under the same hypothesis, Minkowski geometry emerges as the stable effective
      description of a maximally symmetric admissible regime.
      The emergence of the Lorentzian structure is thus not postulated as a background,
      but results, under [H-hyp], from
  ...
  \subsection{Status table}
    ...
    \begin{itemize}
      ...
      \item \textbf{Spatial limit input} [H-L] (Hypothesis~\ref{hyp:HL}): explicit
      hypothesis; not established, and shown by Q5a version 3.0 to fail for the published
      normalisation and its common scalar rescalings.
      \item \textbf{Operator lifting} [H-lift] (Hypothesis~\ref{hyp:lift}): proved in
      Q9~\cite{Beau2026q9}; its object exists only under [H-L].
      \item \textbf{Metric extraction} (Theorem~\ref{thm:metric}): structural consequence
      of [H-lift] (proved in Q9), conditional on [H-L].
      \item \textbf{Lorentzian signature} (Theorem~\ref{thm:lorentz}): imported from the
      conditional signature-selection analysis of~\cite{Beau2026c}, conditional on
      Theorem~\ref{thm:metric} and on the hyperbolicity hypothesis [H-hyp]
      of~\cite{Beau2026c}.
    \end{itemize}
    ...
    \begin{center}
      \begin{tabular}{p{3.2cm}p{6.0cm}p{4.8cm}}
        ...
        {[H-L]} & Spatial limit $\LPi = -A\partial_x^2$ exists &
        Hypothesis; not established (Q5a 3.0 no-go) \\
        {[H-lift]} & $\LPi$ is kinetic sector of $d\pi_1(\DeltaH)$ &
        Proved (Q9~\cite{Beau2026q9}); object conditional on [H-L] \\
        Thm.~\ref{thm:metric} & Metric tensor $g^{\mu\nu} \propto A^{\mu\nu}$ from symbol &
        Conditional on [H-L] (lift: Q9~\cite{Beau2026q9}) \\
        Thm.~\ref{thm:lorentz} & Lorentzian signature $(-,+,+,+)$ &
        Conditional on Thm.~\ref{thm:metric} and [H-hyp] of~\cite{Beau2026c} \\
        \hline
  ...
  \subsection{Ancillary questions and their resolution}
    ...
    $d\pi_\xi(\DeltaH)$ unconditionally, making the lift to $L_{\mathrm{eff}}$
    a theorem rather than a hypothesis.
    Theorem~\ref{thm:metric} is therefore unconditional with respect to~[H-lift], but it
    remains conditional on [H-L]: the existence of the spatial limit operator itself is not
    established (Q5a version 3.0).
    ...
    \]
    The coefficients of the effective co-metric are therefore uniquely determined with
    no free parameter, and, conditionally on [H-L] and on the hyperbolicity hypothesis
    [H-hyp] of~\cite{Beau2026c} (Theorem~\ref{thm:lorentz}), the signature is Lorentzian:
    \[
      g^{\mu\nu} = 2\,\eta^{\mu\nu}, \qquad \eta = \mathrm{diag}(-1,+1,+1,+1).
    \]
    Spacetime structure would be, under these hypotheses, a derived consequence of the
    non-injective projection; establishing [H-L] --- or replacing it --- is the open content
    of Q5.
