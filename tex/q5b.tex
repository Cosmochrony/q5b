...
\begin{document}
  ...
  \begin{abstract}
    ...
    Second, under an explicit lifting hypothesis~[H-lift], we identify $\LPi$ as the
    image, under the Schr\"{o}dinger representation, of the kinetic sector of the sub-Riemannian
    Laplacian $\DeltaH$ on $\HeisR$, and extract an effective contravariant co-metric tensor
    from the principal symbol
    $g^{\mu\nu} \propto A^{\mu\nu}$ from the principal symbol of the full effective operator
    ...
    Each result is given with an explicit status: structural, conditional on [H-lift], or
  ...
  \section{Introduction}
    ...
    \subsection{Context and what Q5a establishes}
      ...
      The Foundation paper~\cite{Beau2026fm} derives, from four axioms governing admissible
    ...
    \subsection{Scope and main results}
      ...
      \medskip
      \noindent\textbf{Metric extraction (Theorem~\ref{thm:metric}; conditional on [H-lift]).}
      Under Hypothesis~[H-lift] (Definition~\ref{hyp:lift}), $\LPi = -A\partial_x^2$ is
...
tested primes.
  ...
  \section{Lifting Hypothesis and Sub-Riemannian Operator}
    ...
    \subsection{The Schr\"{o}dinger representation and the 1D shadow}
      ...
      but we do not prove [H-lift] here.
    ...
    \subsection{Admissibility filter and sector dominance}
      ...
      \begin{itemize}
        ...
        $A_n^{\max} = c_\chi / \sqrt{\lambda_n}$ (BI admissibility envelope).
        \item In the admissible sector, $\langle k^2 \rangle \lesssim (c_\chi)^2 / \lambda_{\min}$
    ...
    \subsection{The lifting hypothesis}
      ...
      \begin{remark}[Justification of [H-lift]]
  ...
  \section{Principal Symbol and Effective Metric Tensor}
    ...
    \subsection{The full effective operator on $\mathbb{R}_\tau \times \HeisR$}
      ...
      \begin{remark}[Relation to the 1D shadow]
...
This confirms the reading of $\LPi$ as the ``one-dimensional shadow'' stated in Q5a.
  ...
  also~\cite{Beau2026a}), the effective metric tensor $g^{\mu\nu}(p)$ is read off
      ...
      \begin{theorem}[Metric extraction; conditional on {[H-lift]} and Q5a hypotheses]
        \label{thm:metric}
        Under Hypothesis~[H-lift] and the Q5a hypotheses {\rm [H1]}, {\rm [H-w]},
          {\rm [H-E1]}, {\rm [C]}, the Mosco limit operator $\LPi = -A\partial_x^2$ lifts
        ...
        co-metric tensor
        \begin{equation}
          g^{\mu\nu}(p) = \mathrm{diag}(-A_\tau,\; A_H,\; A_H,\; 0_z\text{-corrected})
      ...
      \begin{remark}[Status of the $z$-direction]
        ...
        A complete 4D Lorentzian metric on $\mathbb{R}_\tau \times \HeisR$ requires an extension
        of the symbol framework to incorporate the sub-Riemannian corrections from the
        $z$-direction; this is deferred to open problem Q5b-O2.
...
\begin{enumerate}
  ...
  capacity bound $A_n^{\max} = c_\chi / \sqrt{\lambda_n}$~\cite{Beau2026fm,Beau2026c}.
...
Foundation axioms~\cite{Beau2026fm}.
...
of the framework.
      ...
      \begin{remark}[Emergence, not postulation]
        ...
        the Heisenberg symmetry group~\cite{Beau2026fm,Beau2026hs} and the BFS shell
  ...
  \section{Status Table and Open Problems}
    \label{sec:status}
    ...
    \subsection{Open problems}
      ...
      Use the Plancherel formula for $\HeisR$, which decomposes $L^2(\HeisR)$ as a direct
      integral $\int^\oplus_{\xi \neq 0} \mathcal{H}_\xi\,|\xi|\,d\xi$ of Schr\"{o}dinger
      representation spaces $\mathcal{H}_\xi \cong L^2(\mathbb{R})$, to track how the
      admissibility filter $\Pi_q$ acts on each fiber.
      The key step is showing that $\Pi_q$ concentrates admissible modes on the kinetic
      sector $\partial_k^2$ of $d\pi_\xi(\DeltaH)$, i.e., that the potential energy
      $\|(2\pi\xi k)f\|^2$ of admissible functions $f = \Pi_q g$ is bounded by the
      BI capacity envelope, making the kinetic sector dominant in the Mosco limit.
      Expected difficulty: moderate, given T2 of Q5a.

      \textbf{Q5b-O2} (Extension to a full $4 \times 4$ Lorentzian metric).
      Theorem~\ref{thm:metric} and the signature argument of Theorem~\ref{thm:lorentz}
      concern only the non-degenerate part of the sub-Riemannian symbol.
      A complete treatment requires incorporating the $z$-direction, which enters the
      effective metric through sub-principal corrections.
      The natural framework is the theory of hypoelliptic operators and Beals--Greiner
      calculus, or alternatively the tangent groupoid construction for sub-Riemannian
      spaces.
      A full $4 \times 4$ Lorentzian metric on $\mathbb{R}_\tau \times \HeisR$ would
      require showing that the $z$-correction is positive-definite and contributes a
      third spatial direction to the effective geometry.
      Expected difficulty: hard; requires analysis of the sub-principal symbol of
      $L_{\mathrm{eff}}$.

      \textbf{Q5b-O3} (Identification of $A_\tau$ and $A_H$ from admissibility data).
      Theorem~\ref{thm:metric} introduces two positive constants $A_\tau$ and $A_H$
      characterising the temporal and spatial sectors of $L_{\mathrm{eff}}$.
      Their identification from the admissibility data would require: expressing $A_H$
      in terms of the Born--Infeld saturation constant $c_\chi$ and the spectral weight
      $A$ of Q5a (see Q5a-O5); and expressing $A_\tau$ in terms of the projective
      temporal ordering data from~\cite{Beau2026pto}.
      This identification is the first step toward an explicit metric from first principles
      and connects naturally to the Schwarzschild exterior solution derived
      in~\cite{Beau2026c} Section~7.

      The emergence of Lorentzian geometry established in this paper is thus traced back
      entirely to admissibility constraints on projected spectral data, rather than to
      any fundamental geometric postulate; spacetime structure is a derived consequence
      of the non-injective projection, not an input.
  ...
  \section*{Acknowledgements}
    ...
    \bibliography{cosmochrony-bibliography,q5b_supplement}
