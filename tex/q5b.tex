...
\begin{enumerate}
  ...
  capacity bound $A_n^{\max} = c_\chi / \sqrt{\lambda_n}$~\cite{Beau2026m,Beau2026c}.
...
of the framework.
  ...
  \subsection{Status table}
    ...
    \begin{center}
      \begin{tabular}{p{3.2cm}p{6.0cm}p{4.8cm}}
        ...
        Partial: $A_H=2$ (Q10~\cite{Beau2026q10}+U1~\cite{Beau2026u1}); $A_\tau$ open \\
        \hline
      \end{tabular}
    \end{center}
  ...
  \subsection{Resolved problems}
    ...
    Combined with $A_H = 2$ (Q5b-O3 below), the effective co-metric is thus
    $g^{\mu\nu} = \mathrm{diag}(-A_\tau,\, 2,\, 2,\, 2)$, with only $A_\tau$ remaining.
  ...
  \subsection{Remaining open problem}

    \textbf{Q5b-O3} (Identification of $A_\tau$ from admissibility data).
    The identification $A_H = 2$ has been established by Q10~\cite{Beau2026q10} via
    the spectral universality hypothesis~[U] proved in U1~\cite{Beau2026u1}:
    the admissibility weight satisfies $a_q(s)\cdot q^2 \to 2$ as $q\to\infty$
    (confirmed numerically for $q \in \{29,61,101,151,211\}$, W1~\cite{Beau2026w1}),
    giving $A_H = 2$ in the $C_q$ normalisation.
    The single remaining open question is the identification of $A_\tau$ in terms of
    the projective temporal ordering data from~\cite{Beau2026pto}.
    This is the sole scalar needed for a fully explicit metric
    $g^{\mu\nu} = \mathrm{diag}(-A_\tau,\, 2,\, 2,\, 2)$ from first principles.
